...
export default function DashboardPage() {
  ...
  const totalVolume = transfers.reduce((s: number, t: Transfer) => s + t.amount, 0);
  const totalTransfers = transfers.length;
  const avgRiskScore =
    totalTransfers > 0
      ? Math.round(transfers.reduce((s: number, t: Transfer) => s + t.kytScore, 0) / totalTransfers)
      : 0;
  const complianceRate =
    totalTransfers > 0
      ? Math.round((transfers.filter((t: Transfer) => t.kytPassed).length / totalTransfers) * 100)
      : 0;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <div className="mb-6 p-4 rounded-xl border border-amber-200 bg-amber-50 flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-700 mt-0.5">
                Enter your institutional API key to load live transfer data, or set{" "}
        ...
        <div className="card p-0 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            {transfers.length > 0 && (
              <span className="text-xs text-gray-400">{transfers.length} records</span>
            )}
          </div>
          ...
          {!loading && hasKey && transfers.length === 0 && !error && (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              ...
              <p className="text-xs mt-1">Transfers submitted via the Send page will appear here.</p>
            </div>
          )}
          ...
          {!loading && !hasKey && (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              ...
              <p className="text-sm font-medium">Connect an API key to see data</p>
            </div>
          )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    ...
                    <th className="text-left font-medium text-gray-500 px-6 py-3">Transfer</th>
                    <th className="text-right font-medium text-gray-500 px-6 py-3">Amount</th>
                    <th className="text-center font-medium text-gray-500 px-6 py-3">KYT Score</th>
                    <th className="text-center font-medium text-gray-500 px-6 py-3">Travel Rule</th>
                    <th className="text-center font-medium text-gray-500 px-6 py-3">Status</th>
                          ...
                          <tr className="bg-gray-50/60">
                            <td colSpan={7} className="px-6 py-4">
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs animate-fade-in">
                                <div>
                                  <p className="text-gray-500 font-medium mb-1">Sender</p>
                                  <p className="font-mono text-gray-700 break-all">
                                    {tx.senderWallet}
                                  </p>
                                </div>
                                <div>
                                  <p className="text-gray-500 font-medium mb-1">Receiver</p>
                                  <p className="font-mono text-gray-700 break-all">
                                    {tx.receiverWallet}
                                  </p>
                                </div>
                                <div>
                                  <p className="text-gray-500 font-medium mb-1">Compliance Details</p>
                                  <div className="space-y-1">
                                    <p>
                                      <span className="text-gray-500">KYT Score: </span>
                                      <span className="text-gray-900 font-medium">{tx.kytScore}/100</span>
                                    </p>
                                    {tx.travelRuleHash && (
                                      <p>
                                        <span className="text-gray-500">TR Hash: </span>
